...
const router = Router();
const articleDb = new articleRepository();
...
router.post('/',authMiddleware, async (req:Request, res:Response) => {
  ...
  try{
    ...
    const Article:article = new article(user_id,name,quantity,unit);
    await articleDb.createArticle(Article)
...
router.get('/:id',authMiddleware,async (req:Request, res:Response)=>{
    ...
    try{
      ...
      const article = await articleDb.getArticle(Number(id));
...
router.delete('/:id_article',authMiddleware,async (req:Request, res:Response)=>{
  ...
  try{
    ...
    await articleDb.deleteArticle(Number(id_article));
